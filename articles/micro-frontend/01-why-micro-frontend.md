...
- 正在维护中大型后台系统，开始感受到单体应用协作压力的前端同学
- 团队准备讨论微前端，但还不确定是否真的值得引入的技术负责人
- 想从真实案例出发，理解微前端“为什么会出现”的同学
...
为了后面读起来更顺，我们先把几个最常见的术语说清楚：

- **主应用 / 容器应用**：负责系统入口、菜单、权限、登录态、路由分发等全局能力的应用
- **子应用**：按业务边界拆出来的独立前端应用，可以有自己的仓库、技术栈和发布节奏
- **路由分发**：用户访问不同路径时，由主应用决定当前应该加载哪个子应用
- **应用通信**：父子应用或子应用之间传递登录态、事件、上下文信息的机制
- **样式隔离**：避免多个子应用同时运行时出现样式污染和全局 CSS 冲突
- **独立部署**：某个子应用改动后，可以不依赖整个大系统统一发版，单独上线
...
> 图注：前端架构不是凭空走向微前端的，它是随业务复杂度、团队规模和技术债务一步步演进的结果。

早期系统规模小、页面少、团队小，一个单体应用通常就够了。但随着业务线增多、团队人数增加、技术债务累积、交付频率提升，单体架构会逐渐暴露出边界模糊、协作低效、升级困难的问题。微前端就是在这个背景下被推到台前的。

## 四、微前端解决了哪些问题

我这里列两个真实的生产案例，分别对应两类很典型的场景：一类是**多系统整合**，另一类是**老单体系统拆分与升级**。看看微前端在这些场景里到底解决了什么问题。
...
当时大多数管理后台是 Vue 2，也有少部分不是前后端分离的系统，比如 JSP、Laravel（Blade 服务端渲染）这类前后端没有彻底分离的架构。时间大概在 2021 年左右，市面上已经能用到 `qiankun` 这类方案（无界 `Wujie` 要晚到 2022 年年中才开源）。但从改造成本、边界控制、时间要求和团队熟悉度综合考虑，最后采用的是 `iframe` 方式接入。
...
当然，选 `iframe` 也不是没有代价。跨域场景下登录态和第三方 Cookie 常常受限，URL、路由不容易与主应用保持同步，弹窗层级、滚动条这类细节容易露出“拼接感”；更关键的是，每次进入子页面本质上都是重建一次浏览上下文，白屏和资源重载在所难免。这些局限决定了 `iframe` 适合“边界清晰、交互相对独立”的整合；一旦两个页面需要高频联动，通信和体验成本就会迅速放大——这方面的取舍，番外篇一和第三篇会展开细讲。
...
![代码统计示意：项目代码量已超过 50 万行、接近 2000 个文件](assets/01-code-stats.png)
...
如果你的目标只是想解耦构建与协作，而不是真的要拆出多个独立运行的子应用，那“模块化单体 + Monorepo”通常是比微前端更轻的起点：在同一个仓库里按模块/包分层管理，用私有 npm 或构建工具控制依赖边界，往往就能解决一大半“协作”问题（第十三篇会专门讲）。

再举一个看起来很像、实际不建议上微前端的反例：假设你们有 4 个团队、6 条业务线，规模上完全够得着后文第七节的红色区间；但几个团队共享同一套订单领域模型和实时库存状态，页面之间需要高频联动——改一单，列表、详情、看板必须同时刷新，公司还要求全局统一主题和交互。这种系统硬拆开，子应用之间反而天天在做跨应用状态同步，通信和一致性成本比在单体里协作还高。这类项目要优先设计的是领域模型与共享机制，而不是急着把代码“拆出去”。
...
先交代两个口径。其一，阈值是经验值而非行业标准，参考的大致是这类拐点：团队超过 30 人后沟通成本明显上升、构建超过 15 分钟开始阻塞发布节奏、代码量到 50 万行后认知负担集中爆发——不同业务请按自身情况缩放，不必死守数字。其二，当几个维度红绿不一致时（比如构建时间红、但只有一种技术栈且是绿的），建议优先相信“结构性指标”：维度一（团队与组织）和维度四（发布频率）由组织形态决定，一旦进入红色方向性最强；而“构建慢、代码乱”这类症状型指标，多数时候应该先回到上一节排查工程化基础，而不是直接跳到微前端。

```mermaid
flowchart TD
    A[开始评估] --> B{多个相对独立的业务线或团队?}
    B -- 否 --> C[先补齐单体架构与工程化]
    B -- 是 --> D{独立发布或发布频率差异明显?}
    D -- 否 --> E{多技术栈并存或渐进重构需求?}
    D -- 是 --> F[值得重点评估微前端]
    E -- 否 --> G{构建慢、协作冲突、认知负担过高?}
    E -- 是 --> F
    G -- 否 --> C
    G -- 是 --> F
```

> 图注：“值得重点评估微前端”不等于“立即引入”。图中的“构建慢、协作冲突”属于症状型指标，需要结合团队、业务形态一起看——如果工程化基础还没补齐（见第六节），正确的动作是先回到单体里补课，而不是马上拆。
...
**参考场景（示意性示例，细节已做脱敏）：** 某头部 SaaS 公司的管理后台，AngularJS、Backbone、React 三代技术栈并存，最后通过微前端完成了长达 3 年的渐进式迁移，旧代码逐步下线，用户基本无感知。
...
> 图注：典型后台系统的三层逻辑关系——主应用承担系统级能力，子应用承载业务级能力，共享层负责跨项目复用与治理。
...
> 图注：渐进式迁移的推荐节奏——先立好业务边界与主应用基座，再按业务模块逐块迁移，而不是一次性推倒重写。

这条路线的重点不是一次性完成，而是让系统在不断交付业务的同时，逐步走向更清晰的边界和更合理的治理结构。

## 十、真正落地前，建议先回答这 5 个问题

如果你打算把这篇文章当成团队内部讨论材料，我建议在正式启动微前端之前，先把下面 5 个问题回答清楚：

1. 你们要拆分的是“页面”，还是“业务边界”？
2. 子应用之间是否真的需要独立发布，还是只是想缓解当前单体项目的维护压力？
3. 主应用到底负责哪些系统级能力，哪些能力必须沉到共享层？
4. 主子应用之间准备怎么通信，出了问题谁来排查、谁来兜底？
5. 监控、回滚、兼容性和发布规则，有没有在设计阶段就一起想清楚？

如果这 5 个问题回答不清楚，微前端大概率只会把复杂度换一种方式继续保留下来。
...
下一篇《微前端核心概念与设计理念》，我们会回答“到底该怎么理解微前端”：它的四个关键特征是什么、为什么值得引入、又会带来哪些代价，以及业务系统到底应该按什么方式拆分，才不会“拆了等于没拆”。
